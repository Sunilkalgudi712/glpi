...
TWIG,
    ...
    public static function generateMenuSession($force = false)
    {
        ...
        if (
            ...
            // Ensure Assistance menu titles
            if (isset($menu['helpdesk']['content'])) {
                if (isset($menu['helpdesk']['content']['ticket'])) {
                    $menu['helpdesk']['content']['ticket']['title'] = Ticket::getTypeName(2);
                }
                if (isset($menu['helpdesk']['content']['create_ticket'])) {
                    $menu['helpdesk']['content']['create_ticket']['title'] = __('Create job');
                }
                if (isset($menu['helpdesk']['content']['ticketrecurrent'])) {
                    $menu['helpdesk']['content']['ticketrecurrent']['title'] = TicketRecurrent::getTypeName();
                }
            }
